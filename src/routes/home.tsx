...
export default function Home() {
  ...
  const goBuild = (query: string, mode: AgentMode) => {
    const q = (query?.trim() || ideas[i]).trim();
    const intendedUrl = `/chat/new?query=${encodeURIComponent(q)}&agentMode=${encodeURIComponent(mode)}`;

    if (!requireAuth({ requireFullAuth: true, actionContext: 'to create applications', intendedUrl })) {
      return; // will redirect to auth if needed
    }
  ...
  useEffect(() => {
    const phrase = ideas[i] || '';
    if (typing) {
      if (typed.length < phrase.length) {
        const t = setTimeout(() => setTyped(phrase.slice(0, typed.length + 1)), 60);
      ...
      if (typed.length) {
        ...
        setI((p) => (p + 1) % ideas.length);
        setTyping(true);
      }
    }
  }, [typed, typing, i, ideas]);
    ...
    <div className="relative flex flex-col items-center w-full">
      ...
      <section className="w-full max-w-5xl px-6 pt-20 md:pt-28 pb-12 md:pb-16">
        <h1 className="text-text-primary font-extrabold leading-[1.05] tracking-tight text-5xl md:text-7xl">
          Build your own <br></br><span className="text-accent">micro-SaaS</span> apps
        ...
        <form
          ...
          onSubmit={(e) => { e.preventDefault(); goBuild(textareaRef.current?.value || '', agentMode); }}
        >
              ...
              <textarea
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                    goBuild(textareaRef.current?.value || '', agentMode);
                  }
                }}
